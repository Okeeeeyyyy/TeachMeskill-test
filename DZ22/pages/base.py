...
from DZ22.data.environment import host
...
class Base:
    ...
    def get_text(
        self, locator: str, index: int
    ) -> str:  # достаем текст, если локатор один, то в аргумент прокидываем значение 0
    ...
    def input_value_by_index(
        self, locator: str, index: int, data: str
    ) -> None:  # вводим данные в нужные поля по индексу
    ...
    def open_new_tab_and_check_presence(
        self, locclick, locpresence
    ...
    def switch_to_iframe_and_click(
        self, iframe_locator, locator_for_click
    ...
    def switch_to_iframe_and_input(
        self, iframe_locator, locator_for_input, data: str
