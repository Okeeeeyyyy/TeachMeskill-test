...
def get_chrome_browser(playwright, request) -> Browser:
    return playwright.chromium.launch(
        headless=request.config.getoption("h"), slow_mo=request.config.getoption("slow"), args=["--start-maximized"]
    )


def get_remote_chrome(playwright, request) -> Browser:
    return playwright.chromium.launch(headless=True, slow_mo=request.config.getoption("slow"))


def get_context(browser, request, start) -> BrowserContext:
    if start == "local":
        context = browser.new_context(no_viewport=True, locale=request.config.getoption("l"))
        context.set_default_timeout(timeout=request.config.getoption("t"))
        return context

    elif start == "remote":
        context = browser.new_context(viewport=request.config.getoption("s"), locale=request.config.getoption("l"))
        context.set_default_timeout(timeout=request.config.getoption("t"))
        return context
